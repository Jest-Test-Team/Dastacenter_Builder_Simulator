...
describe('scoring engine', () => {
  ...
  it('returns a high score for a redundant, cooled, secured build', () => {
    const s = makeState();
    for (let i = 0; i < 4; i++) placeBlock(s, { typeId: 'server_rack', cell: { x: 4 + i, y: 1, z: 4 } });
    placeBlock(s, { typeId: 'utility_feed', cell: { x: 0, y: 1, z: 4 } });
    placeBlock(s, { typeId: 'utility_feed', cell: { x: 0, y: 1, z: 5 } });
    placeBlock(s, { typeId: 'ups', cell: { x: 1, y: 1, z: 4 } });
    placeBlock(s, { typeId: 'generator', cell: { x: 1, y: 1, z: 5 } });
    placeBlock(s, { typeId: 'crac_unit', cell: { x: 4, y: 1, z: 3 } });
    placeBlock(s, { typeId: 'fire_panel', cell: { x: 0, y: 1, z: 0 } });
    placeBlock(s, { typeId: 'sprinkler', cell: { x: 4, y: 2, z: 4 } });
    placeBlock(s, { typeId: 'cctv', cell: { x: 16, y: 0, z: 0 } });
    placeBlock(s, { typeId: 'mantrap', cell: { x: 0, y: 1, z: 16 } });
    placeBlock(s, { typeId: 'bollard', cell: { x: 16, y: 0, z: 16 } });
  ...
  it('is deterministic', () => {
    const s1 = makeState();
    const s2 = makeState();
    placeBlock(s1, { typeId: 'server_rack', cell: { x: 4, y: 1, z: 4 } });
    placeBlock(s2, { typeId: 'server_rack', cell: { x: 4, y: 1, z: 4 } });
    const r1 = score(s1);
    const r2 = score(s2);
